...
export default class BuyerLocations {
    ...
    */
    public async Create(buyerID: string,  accessToken?: string ): Promise<RequiredDeep<MarketplaceBuyerLocation>> {
        const impersonating = this.impersonating;
        this.impersonating = false;
        return await httpClient.post(`/buyerlocations/${buyerID}`, { params: {  accessToken, impersonating } } );
    }
    ...
    * @param accessToken Provide an alternative token to the one stored in the sdk instance (useful for impersonation).
    */
    public async Update(buyerLocationID: string, buyerID: string,  accessToken?: string ): Promise<RequiredDeep<MarketplaceBuyerLocation>> {
        const impersonating = this.impersonating;
        this.impersonating = false;
        return await httpClient.put(`/buyerlocations/${buyerID}/${buyerLocationID}`, { params: {  accessToken, impersonating } } );
    }
