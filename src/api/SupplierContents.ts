...
export default class SupplierContents {
    ...
    * @param accessToken Provide an alternative token to the one stored in the sdk instance (useful for impersonation).
    */
    public async ListDocuments(supplierID: string, schemaID: string,  options: ListArgs<Document> = {}, accessToken?: string ): Promise<RequiredDeep<ListPage<Document>>> {
        const impersonating = this.impersonating;
        this.impersonating = false;
        return await httpClient.get(`/suppliers/${supplierID}/schemas/${schemaID}/documents`, { params: { ...options,  filters: options.filters, accessToken, impersonating } } );
    }
    ...
    * @param accessToken Provide an alternative token to the one stored in the sdk instance (useful for impersonation).
    */
    public async SaveDocumentAssignment(supplierID: string, schemaID: string, documentID: string,  accessToken?: string ): Promise<void> {
        const impersonating = this.impersonating;
        this.impersonating = false;
        return await httpClient.post(`/suppliers/${supplierID}/schemas/${schemaID}/documents/${documentID}/assignments`, {}, { params: {  accessToken, impersonating } } );
    }
    ...
    * @param accessToken Provide an alternative token to the one stored in the sdk instance (useful for impersonation).
    */
    public async DeleteDocumentAssignment(supplierID: string, schemaID: string, documentID: string,  accessToken?: string ): Promise<void> {
        const impersonating = this.impersonating;
        this.impersonating = false;
        return await httpClient.delete(`/suppliers/${supplierID}/schemas/${schemaID}/documents/${documentID}/assignments`, { params: {  accessToken, impersonating } } );
    }
